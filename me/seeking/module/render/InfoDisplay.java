...
public class InfoDisplay extends Module {
    ...
    public void on2D(EventRender2D e){
        setWidth(8+FontLoaders.fontBig18.getStringWidth("SEEKING ")+FontLoaders.font18.getStringWidth("| "+mc.getSession().getUsername()+" | PING:"+mc.getMinecraft().getNetHandler().getPlayerInfo(mc.getMinecraft().thePlayer.getUniqueID()).getResponseTime()+" | "+LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss"))));
