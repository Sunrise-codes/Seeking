 ...
 */
public abstract class GLSLSandBox {
    private int program;

    private Map<String, Integer> uniformsMap;
    public final static Minecraft mc = Minecraft.getMinecraft();

    public GLSLSandBox(final String fragmentShader) {
        int vertexShaderID, fragmentShaderID;

        try {
            final InputStream vertexStream = getClass().getResourceAsStream("/assets/minecraft/seeking/vertex.vert");
            vertexShaderID = createShader(IOUtils.toString(vertexStream), ARBVertexShader.GL_VERTEX_SHADER_ARB);
            IOUtils.closeQuietly(vertexStream);

            final InputStream fragmentStream = getClass().getResourceAsStream("/assets/minecraft/seeking/shaders/" + fragmentShader);
